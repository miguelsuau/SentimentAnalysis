...
def main():
  ...
  # split samples into a positive and a negative list
    review = row[0]
    topic = row[2]
    if topic == 'general issues':
      issues_reviews.append(review)
    elif topic == 'delay':
      delay_reviews.append(review)
  # close file
  processed.close()
  # randomly split data into training and testing sets
  tr_pos,tr_neg,t_pos,t_neg = random_split(issues_reviews,delay_reviews)
  # convert list of strings into list of words
  ...
  # Transform training set into array of integers
  ...
  # Transform testing set into array of integers
